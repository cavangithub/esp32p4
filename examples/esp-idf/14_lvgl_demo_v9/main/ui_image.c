...
lv_obj_t *photo_page_create(void)
{
    ...
    lv_obj_set_size(ui->status_bar,
                    LV_HOR_RES,
                    32);

    lv_obj_align(ui->status_bar,
                 LV_ALIGN_TOP_MID,
                 0,
                 0);

    lv_obj_set_style_bg_color(ui->status_bar,
                              lv_color_black(),
                              0);

    lv_obj_set_style_bg_opa(ui->status_bar,
                            LV_OPA_50,
                            0);

    ui->label_usb = lv_label_create(ui->status_bar);
    lv_label_set_text(ui->label_usb,
                      "USB Connected");

    lv_obj_align(ui->label_usb,
                 LV_ALIGN_LEFT_MID,
                 10,
                 0);
    ...
    lv_obj_set_size(ui->ctrl_panel,
                    LV_HOR_RES,
                    60);

    lv_obj_align(ui->ctrl_panel,
                 LV_ALIGN_BOTTOM_MID,
                 0,
                 0);

    lv_obj_set_style_bg_color(ui->ctrl_panel,
                              lv_color_black(),
                              0);

    lv_obj_set_style_bg_opa(ui->ctrl_panel,
                            LV_OPA_50,
                            0);

    ui->btn_prev = lv_button_create(ui->ctrl_panel);
    lv_obj_set_size(ui->btn_prev, 100, 40);
    lv_obj_align(ui->btn_prev,
                 LV_ALIGN_LEFT_MID,
                 20,
                 0);
    ...
    lv_obj_t *label_play = lv_label_create(ui->btn_play);
